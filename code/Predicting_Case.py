...
MODEL_PATH = "models/32_400/checkpoint_epoch_350.pth"                  # 训练好的模型
CSV_PATH   = "dataset/testing/test_200.csv"       # 你的占用情况 CSV
IMG_DIR    = "dataset/plan"                 # bmp 平面图所在文件夹
OUTPUT_CSV = CSV_PATH + "simulated_seats_with_control.csv"  # 输出文件名
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        # 写回这行的 task_light*
        for i in range(24):
            # 获取该座位当前的真实占用情况 (0 或 1)
            seat_is_occupied = (row[seat_cols[i]] == 1)
            
            # 如果没人坐，强制设为 0；如果有人坐，则使用模型的预测值
            final_task_val = int(tasks_rounded[i]) if seat_is_occupied else 0
            df.at[idx, task_light_cols[i]] = final_task_val
